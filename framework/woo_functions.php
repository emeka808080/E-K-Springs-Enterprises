<?php 
/*** Tiny account ***/
if( !function_exists('druco_tiny_account') ){
	function druco_tiny_account( $show_dropdown = true ){
		$login_url = '#';
		$register_url = '#';
		$profile_url = '#';
		...
		<div class="ts-tiny-account-wrapper">
			<div class="account-control">
				...
				<?php if( !$_user_logged ): ?>
					<a class="login" href="<?php echo esc_url($login_url); ?>" title="<?php esc_attr_e('My Account', 'druco'); ?>"><?php echo esc_html__( 'Login/Register', 'druco' ); ?></a>
				<?php else: ?>
					<a class="my-account" href="<?php echo esc_url($profile_url); ?>" title="<?php esc_attr_e('My Account', 'druco'); ?>"><?php echo esc_html__( 'My Account', 'druco' ); ?></a>
				<?php endif; ?>
					...
					<div class="form-content">
						...
						<?php if( !$_user_logged ): ?>
							<?php wp_login_form( array(
								...
								'label_username' 	=> '',
								'label_password'	=> '',
								'label_log_in'		=> esc_html__( 'Sign in', 'druco')
							) ); ?>
						<?php else: ?>
							<ul>
								<?php do_action('druco_before_my_account_dropdown_list'); ?>
								<li><a class="my-account" href="<?php echo esc_url($profile_url); ?>"><?php esc_html_e( 'My Account', 'druco' ); ?></a></li>
								<li><a class="log-out" href="<?php echo esc_url($logout_url); ?>"><?php esc_html_e( 'Logout', 'druco' ); ?></a></li>
							</ul>
						<?php endif; ?>
...
/*** Tiny Cart ***/
if( !function_exists('druco_tiny_cart') ){
	function druco_tiny_cart( $show_cart_control = true, $show_cart_dropdown = true ){
		if( !class_exists('WooCommerce') ){
			return '';
		}
		...
		?>
			<div class="ts-tiny-cart-wrapper">
				...
				<div class="cart-icon">
					<a class="cart-control" href="<?php echo esc_url($cart_url); ?>" title="<?php esc_attr_e('View your shopping cart', 'druco'); ?>">
				...
				<div class="cart-dropdown-form dropdown-container woocommerce">
					<div class="form-content">
						<?php if( $cart_empty ): ?>
							...
							<span><?php esc_html_e('Your cart is currently empty', 'druco'); ?></span></label>
						<?php else: ?>
							...
							<div class="cart-wrapper">
								<div class="cart-content">
									...
									<div class="dropdown-footer">
										...
										<a href="<?php echo esc_url($cart_url); ?>" class="button view-cart"><?php esc_html_e('View Cart', 'druco'); ?></a>
										<a href="<?php echo esc_url($checkout_url); ?>" class="button checkout-button"><?php esc_html_e('Checkout', 'druco'); ?></a>
									</div>
								</div>
							</div>
						<?php endif; ?>
...
add_filter('woocommerce_add_to_cart_fragments', 'druco_tiny_cart_filter');
function druco_tiny_cart_filter($fragments){
	$cart_sidebar = druco_get_theme_options('ts_shopping_cart_sidebar');
	$fragments['.ts-tiny-cart-wrapper'] = druco_tiny_cart(true, !$cart_sidebar);
	if( $cart_sidebar ){
		$fragments['#ts-shopping-cart-sidebar .ts-tiny-cart-wrapper'] = druco_tiny_cart(false, true);
	}
	return $fragments;
}

add_action('wp_ajax_druco_update_cart_quantity', 'druco_update_cart_quantity');
add_action('wp_ajax_nopriv_druco_update_cart_quantity', 'druco_update_cart_quantity');
function druco_update_cart_quantity(){
...
function druco_tini_wishlist(){
	...
	?>
	<a title="<?php esc_attr_e('Wishlist', 'druco'); ?>" href="<?php echo esc_url($wishlist_page); ?>" class="tini-wishlist">
...
function druco_update_tini_wishlist() {
	die(druco_tini_wishlist());
}

add_action('wp_ajax_druco_update_tini_wishlist', 'druco_update_tini_wishlist');
add_action('wp_ajax_nopriv_druco_update_tini_wishlist', 'druco_update_tini_wishlist');

if( !function_exists('druco_woocommerce_multilingual_currency_switcher') ){
	function druco_woocommerce_multilingual_currency_switcher(){
		if( class_exists('woocommerce_wpml') && class_exists('WooCommerce') && class_exists('SitePress') ){
			...
			echo '<div class="wcml_currency_switcher">';
				echo wp_kses( $selected_html, 'druco_link' );
				echo '<ul>';
			...
			do_action('druco_header_currency_switcher'); /* Allow use another currency switcher */
		}
	}
}

add_filter( 'wcml_multi_currency_ajax_actions', 'druco_wcml_multi_currency_ajax_actions_filter' );
if( !function_exists('druco_wcml_multi_currency_ajax_actions_filter') ){
	function druco_wcml_multi_currency_ajax_actions_filter( $actions ){
		$actions[] = 'remove_from_wishlist';
		$actions[] = 'druco_ajax_search';
		$actions[] = 'druco_load_quickshop_content';
		$actions[] = 'druco_update_cart_quantity';
		$actions[] = 'druco_load_product_added_to_cart';
		$actions[] = 'ts_get_product_content_in_category_tab';
		return $actions;
	}
}

if( !function_exists('druco_wpml_language_selector') ){
	function druco_wpml_language_selector(){
		if( class_exists('SitePress') ){
			do_action('wpml_add_language_selector');
		}
		else{
			do_action('druco_header_language_switcher'); /* Allow use another language switcher */
		}
	}
}

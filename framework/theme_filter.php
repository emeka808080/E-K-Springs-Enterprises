<?php
/*** Comment ***/
function druco_list_comments( $comment, $args, $depth ){
	...
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
		<div id="comment-<?php comment_ID(); ?>"  class="comment-wrapper">
			...
			<div class="comment-detail">
				...
				<em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'druco' ); ?></em>
				<?php endif; ?>
				...
				<div class="comment-actions">
					...
					<span class="edit"><?php edit_comment_link( esc_html__( 'Edit', 'druco' ), '' ); ?></span>
					<?php endif;?>
	...
	<li class="post pingback">
		<p><?php esc_html_e( 'Pingback:', 'druco' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( esc_html__( '(Edit)', 'druco' ), ' ' ); ?></p>
	<?php
			break;
	endswitch;
}

function druco_comment_form( $args = array(), $post_id = null ){
	...
	$fields =  array(
		'author' 	=> '<p class="author-row"><input placeholder="'. esc_attr__('Name *', 'druco') .'" id="author" class="input-text" name="author" type="text" value="'. $comment_author .'" size="30"' . $aria_req . ' />' . '</p>'
		,'email'	=> '<p class="email-row"><input placeholder="'. esc_attr__('Email *', 'druco') .'" id="email" class="input-text" name="email" type="text" value="'. $comment_author_email .'" size="30"' . $aria_req . '/>' . '</p>'
		,'url'  	=> '<p class="website-row"><input placeholder="'. esc_attr__('Website', 'druco') .'" id="url" class="input-text" name="url" type="text" value="'. $comment_author_url .'" size="30" />' . '</p>'
	);

	$required_text = sprintf( ' ' . wp_kses( __('Required fields are marked %s','druco'), $allowed_html ), '<span class="required">*</span>' );
	$defaults = array(
		'fields'               	=> apply_filters( 'comment_form_default_fields', $fields )
		,'fields_before'		=> '<div class="info-wrapper">'
		,'fields_after'		    => '</div>'
		,'comment_field'        => '<div class="message-wrapper"><p><textarea id="comment" name="comment" cols="45" rows="8" aria-required="true" placeholder="' .esc_attr__('Add your comment..', 'druco'). '"></textarea></p></div>'
		,'must_log_in'          => '<p class="must-log-in">' .  sprintf( __( 'You must be <a href="%s">logged in</a> to post a comment.', 'druco' ), wp_login_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>'
		,'logged_in_as'         => '<p class="logged-in-as">' . sprintf( __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>', 'druco' ), admin_url( 'profile.php' ), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>'
		,'comment_notes_before' => '<p>'. esc_html__( 'Your email address will not be published. Required fields are marked *', 'druco' ) . '</p>'
		,'comment_notes_after'  => ''
		,'id_form'              => 'commentform'
		,'id_submit'            => 'submit'
		,'title_reply'          => esc_html__( 'Leave a comment', 'druco' )
		,'title_reply_to'       => esc_html__( 'Leave a Reply to %s', 'druco')
		,'cancel_reply_link'    => esc_html__( 'Cancel reply', 'druco' )
		,'label_submit'         => esc_html__( 'Post comment', 'druco' )
	);
...
/* kses allowed html */
add_filter('wp_kses_allowed_html', 'druco_wp_kses_allowed_html', 10, 2);
function druco_wp_kses_allowed_html( $tags, $context ){
	switch( $context ){
		case 'druco_tgmpa':
			$tags = array(
...
/* Body classes filter */
add_filter('body_class', 'druco_body_classes_filter');
function druco_body_classes_filter( $classes ){
	...
	if( druco_get_page_options('ts_display_vertical_menu_by_default') ){
